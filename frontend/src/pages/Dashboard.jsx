...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            try {
                const holdingsData = await tradeService.getHoldings();
                setHoldings(Array.isArray(holdingsData) ? holdingsData : []);
            } catch (error) {
                console.error("Failed to fetch holdings:", error);
            }

            // Fetch Market Pulse (Non-critical)
            try {
                const transData = await tradeService.getMarketPulse();
                setTransactions(Array.isArray(transData) ? transData : []);
            } catch (error) {
                console.error("Failed to fetch market pulse:", error);
                // Fallback to empty if pulse fails (backend might be deploying)
                setTransactions([]);
            }
